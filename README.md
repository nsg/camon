...
- **Continuous recording** (`storage.enabled = true`, `analytics.enabled = false`) — "dumb NVR" mode. With no analyzer to gate on motion, a per-camera recorder rolls every segment from the hot buffer to disk in fixed-length chunks (each `max_event_duration_secs` long) under `continuous/`. Every chunk is a whole-GOP `.ts` that decodes on its own, and successive chunks carry a `"continues"` flag so the timeline stitches seamlessly: the web UI reads that flag back and, where a chunk really does start where the one before it ended, rails them together in the event list as one recording rather than listing them as unrelated events. It counts a chunk's position ("part 3") only while the start of that recording is still on disk; once retention has taken it, a chunk says only that it is "continued", because no number the UI could show would be its real place in a recording that is partly gone. This is heavy — roughly **43 GB/day/camera at 4 Mbps** — so `continuous_retention_days` defaults to 1.
...
Retention belongs to one task for the whole store rather than to each camera's writer — a sweep covers every camera, so one task does it however many cameras are configured, instead of a sweep per camera racing all the others — and it runs hourly; on shutdown it stops between events, never part-way through deleting one. An event leaves the index only once its video is actually gone — a delete that fails leaves the event listed and playable and the next sweep retries it, instead of unindexing a recording that is still occupying disk with nothing left to describe it. Within one event the files go in a fixed order, sidecar and thumbnails first and the `.ts` last, because unlinks are no more durable than a rename is: what a power cut mid-delete leaves behind is then an event the next startup scan still finds and the next sweep deletes again, whereas the other order strands a sidecar and a few thumbnails that nothing looks for — retention is driven off `.ts` files and startup recovery sweeps only `.tmp` — and that nothing therefore ever collects. The emergency low-space pass skips events it has already failed to delete, so they cannot stand in front of events that would free space, and space it failed to reclaim is never counted as reclaimed.
...
One fault deliberately falls outside that watch, because nothing about it is silent: `data_dir` being unmounted while camon runs. The mountpoint is still a directory, so writes land on whatever filesystem is behind it — usually the root one — and report success, which keeps resetting the silence watchdog and points the `min_free_bytes` guard at a disk the footage is not on. camon therefore **marks the storage directory at startup** with a small `.camon-volume` file and re-checks it once a minute, in a single `stat`: the file being gone means the tree being written into is not the tree that was initialised — which is how a vanished *bind* mount is caught, since that need not change the device id at all — and the file turning up on a different device means the volume was swapped or something was mounted over the path. Either is reported at error level, repeated hourly while it lasts, and reported again when the volume comes back. It is never fatal: refusing to record would turn footage in the wrong place into no footage at all, and footage in the wrong place can still be moved. What does stop is the emergency prune, because deleting the archive to free space on a filesystem the archive is not on is the one step with no way back. The comparison is only ever against this process's own startup reading, and that reading is never written down, so a first run that creates the directory, a relative `data_dir`, a bind mount, and an operator moving storage to another volume between runs all stay quiet. A stored-event directory that exists but cannot be read is warned about too, rather than scanned past as an empty tier — that is what let an unreadable archive look identical to a fresh install.
...
- **Local disk** (default) — the `data_dir` on the machine running camon, with the atomic write ladder, crash recovery, `min_free_bytes` free-space guard, and startup volume marker described above.
- **Remote stathost** — a [stathost](https://github.com/nsg/stathost) static file host, selected by adding a `[storage.stathost]` section. Each event becomes three sibling objects under `{camera}/` on the host — the `.ts` video, a `.json` sidecar (which here also carries the event **type**, since there are no `movements/`/`objects/`/`continuous/` directories), and eager `{stem}_thumb_{i}.jpg` filmstrip frames. Time-based retention still applies via `DELETE`; because the client can't see the server's disk, retention-by-space becomes a client-side **budget** (`max_stored_bytes`) that prunes the oldest events (continuous → movements → objects) when tracked usage exceeds it. That eviction follows the local low-space pass where the two backends fail alike and diverges where they don't: an event whose deletion the store has already refused goes to the back of its tier instead of ahead of events that would actually free space, but it is never excluded outright — a pass here stops at the first failure (a refusal is a network answer, not one poisoned file, and each attempt can hold a warm writer for a minute), and excluding as well as stopping would leave an outage's flagged events unreclaimable and the budget permanently over its limit, since the hourly sweep only ever retries events that are already age-expired. Space that was never reclaimed (an object that had already vanished) is counted apart from space that was. Within one event, thumbnails go first, then the video, then the sidecar — the reverse of local disk's order after the video, because the sidecar is the only record of the event's type here: a refused video delete keeps it, so the retry still measures the event against its own retention rather than against whatever a type-less `.ts` would be read as (which is not reliably shorter — `continuous_retention_days` defaults to 1 day against movement's 2, and the three retentions can be configured in any order). Post-hoc movement→object upgrades just rewrite the sidecar in place — no object is ever renamed or moved. An upload to a key that already exists is an *update*, so re-writing an event's stem rewrites that one event: the index replaces its entry rather than gaining a second one for objects that don't exist, and thumbnails the new version has no frame for are deleted.
  ...
  Requires stathost **0.2.0 or later** — camon relies on its atomic uploads (readers never see a partially uploaded object), its detailed listing (`?detail=true`) for accurate storage accounting, and its HTTP Range support; there is no fallback for older servers. Because the sidecar is the only record of an event's **type**, it is uploaded *before* the video, which commits the event — an event whose sidecar cannot be stored is failed rather than written as a video that would scan back as a plain movement and expire on the wrong retention. The one exception is a plain movement event, which is exactly what a sidecar-less `.ts` scans back as anyway. Nothing is rolled back on a failed upload (a failure can still have committed server-side): the leftovers are an orphan sidecar, which indexes nothing, or a video that its sidecar still types correctly. Orphaned metadata is collected by the next startup scan, which already has the full listing — nothing else ever could, since the index is built from `.ts` objects and an event's siblings are only deleted along with it, so on a flaky uplink they would otherwise accumulate for the life of the bucket. The sweep runs before any camera starts recording, never queues a retry, touches only the cameras and object names this camon owns, and re-checks each candidate against the host immediately before deleting it — the listing is a snapshot, and a video can land after it was taken. Deleting only on a *confirmed* absence narrows the race to a single request round-trip; it cannot be closed entirely, because stathost has no conditional delete, and losing it would cost one video its type record (the same state a sidecar that could not be stored already produces), not any footage. Because that scan has to finish before the first camera starts recording, it issues its requests **16 at a time** rather than one after another: it needs a sidecar read per stored event, and awaiting each in turn made startup a function of how much the bucket already holds — thousands of round trips, minutes of them, before anything was being recorded. The width is sized for what stathost is (a small static file host, one HTTP/1.1 connection per request in flight), and only affects when requests go out: the index is still built in listing order, and each read still decides on its own whether it saw a confirmed absence or a failure to find out. Warm-event playback **streams** rather than buffering the whole event in RAM: the playback handler forwards a single `Range` header to stathost and relays the `206`/`Content-Range` response, so seeking fetches only the requested bytes.
...
# The swap itself is made durable like every other file camon has to find
# again: the download is fsynced, and made executable, before it is renamed
# over the running binary, and the directory holding the name is fsynced after.
# A rename is atomic but not durable on its own, so without that a power cut
# moments after an update could leave a truncated or empty `camon` under the
# live name — an installation that no longer starts at all.
...
enabled = true
# Directory for event files (default: /var/camon/storage). Mount a dedicated
# volume here before starting camon: camon marks this directory at startup and
# re-checks the mark once a minute, so losing the volume under a running camon
# is reported rather than silently redirecting writes to the root filesystem.
