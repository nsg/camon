...
/// Marker file name, in `data_dir` itself. Outside every directory the index
/// and the orphan sweep walk (`{data_dir}/{camera}/{tier}/`), so nothing can
/// mistake it for an event.
const MARKER_NAME: &str = ".camon-volume";

/// Contents of the marker, for whoever finds it and wonders. Never read back:
/// the file's existence and the device under it are the whole signal.
...
/// How often the anchor is checked.
pub const POLL_INTERVAL: Duration = Duration::from_secs(60);

/// How long between repeats of a fault that is still there. Long, because the
/// release log is otherwise empty and the message is unactionable more than
/// once an hour.
...
impl StorageAnchor {
    /// Mark `data_dir` and record what it is on.
    ...
    fn arm(&self, state: &mut AnchorState) {
        ...
        // marker is rewritten on every start and never read across one, so a
        // power cut losing it costs nothing.
...
impl AnchorReport {
    /// Loud enough to survive the release filter, but deliberately not fatal: refusing to
    /// record would turn footage in the wrong place into no footage at all. What camon stops
    /// doing is *deleting* — see the low-space guard.
